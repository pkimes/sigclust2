...
\subsection{Citation} 
For a more in depth discussion of the apporach including the multiple testing correction procedure implemented in \CRANpkg{SigClust2}, contact Patrick. The corresponding manuscript is still in preparation.
\begin{quote}
Kimes, P., Liu, Y., Marron, J.S. HSigClust: statistical significance in hierarchical clustering. \textit{In preparation}.
\end{quote}


\section{\texttt{HSCtest} Implementation}
<<>>=
# library(SigClust2)
# generate some data
...
The results of \CRANpkg{HSigClust} may be summarized in a dendrogram.
<<>>=
# plot the results of the implementation from above
# reader: "I like pretty pictures!"
# Patrick: "me too!"
@
